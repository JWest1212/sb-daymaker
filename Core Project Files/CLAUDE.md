...
- **Plan** (a create-action in the bottom bar — building a day, **not** a browse destination): a build-it-yourself single-day spine — set the shape (filters) → land on an **empty time-of-day spine** → fill each slot from a **saved-first picker** (saved items always float to the top band). No auto-generated day, no day-shapes, no slotting engine. **Four core behaviors:** (1) saved-first picker, (2) keep adding freely (new slot appears after each add), (3) time shown only if the thing has a real `starts_at` — never a fabricated range, (4) overlaps are fine, no warnings, insertion order preserved. Save to **Saved › Days** → **view-only share** link. Fully deterministic (pure ranking function over pre-computed `things`); **no AI at tap time**. `lib/plan/buildDay.ts` and `lib/plan/dayShapes.ts` were removed — do not reintroduce them. See `docs/plan-simplification/`.
