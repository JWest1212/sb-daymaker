...
> - **Picker tiles** (the Area, Occasion and Activity sheets) show generated line art on a solid ground in the picker's colour (`scripts/gen-tile-art.mjs` writes `public/tiles/{place,vibe,activity}/*.svg`); re-run it when a tile is added.
> - **Explore:** the value-prop hero over the golden-hour skyline · **Today's pick** above the feed, never blank (§2.5), gated on `hero_eligible`, a real address and a start no more than 30 minutes past · **three doors: Area, Occasion, Activity**, each a sheet of tiles with live counts ("Which part of town?", "What's the occasion?", "What do you want to do?"); zero-count tiles show disabled with their count, never hidden; choices stack as AND filters with removable chips and one **Reset** · **the WHEN row: six tabs in the locked order Today, Tomorrow, Weekend, Next weekend, Week, Month** (D7), in one line that scrolls sideways inside itself, each with its dates shown under the name ("Sep 25-27", "Next 7 days"), and that visible text is its accessible name; the tabs carry no counts, the sheet tiles do (TP-C8-05) · the cascade feed under plain horizon headings ("Today", "This Weekend"; Today keeps events from earlier in the day) in **one card anatomy** on every horizon, each recurring series collapsed to one card (D6) · **header search** (deterministic `/api/search`: dedupes by series then title, excludes civic and archived rows, tolerates a typo with "Did you mean") · filter and horizon state in the URL (`?when=&area=&occasion=&activity=`), read by the server, so a shared link opens on the same view · "Build your day" on Today · the newsletter block ("Santa Barbara, twice a week").
...
- **Explore** (the front door): the value-prop hero over the golden-hour skyline · Today's pick (never blank) · the three doors **Area · Occasion · Activity** · the six-tab WHEN row (**Today, Tomorrow, Weekend, Next weekend, Week, Month**, one scrolling line with dates) · header search · the happenings cascade feed (dated → recurring → evergreen) in one card anatomy, each recurring series collapsed to one card · state in the URL · "Build your day" on Today. **No Near Me on Explore** (D11). *(One Perfect SB Day / "Make My Day" was scrapped 2026-07-04; see Removed.)*
