...
Canon amendments recorded as builds diverge from or extend the canon (v11 since R1 Wave 8)
...
Vocabulary: one name per concept from `lib/strings.ts` (Area, Occasion, Activity, Reset, Saved, Want to go, Been, "N on your list", How it works, Suggest an event or business, Morning, Afternoon, Evening, Newsletter). The doors read Area, Occasion and Activity with the new sheet titles; the detail field is "Area" and "Nearby in [short area]"; category labels are Title Case with one arts label, "Arts & Culture". Header search now pushes the params Explore reads (it pushed ?place= and ?vibe=, which W6.1 had stopped reading, so Area and Occasion results did nothing). Newsletter: heading "Santa Barbara, twice a week", the one cadence string in the signup block, the sample footer, /confirm, /unsubscribe and both emails. Promises: "learns your Santa Barbara", "replay", "Every stop is open when we schedule it", "real hours" and "Quietly building your Santa Barbara" are gone; "Verified" now means `verified_at` only; the guide passport and disabled Been buttons (marking was never built) and the shared plan's "Save this plan" (no screen lists saved plans) are hidden. Welcome: a dismissible strip above the pick replaced the auto-opening modal; `sbd.tour.v1` is written on close; the tour opens from How it works on every page and labels its card as an example. The itineraries gate counted the key's presence, which the plans store writes on every load, so an unclosed strip would have vanished on visit 2; it now counts real plans (D8). D10: no visit-count code ever existed. Icons: emoji in tags, Saved, Discover and not-found pages became the icon set (compass, close and plus added). Dashes: the check scans CSS, JS and public/ too after a purge of 141 comment dashes; the normalizer handles en dashes (numeric range to a hyphen, day, month or clock range to "to", separator to a comma), which cleans 148 live en dashes at render and at write; the AI prompts forbid both (D9). Discover: one name per guide ("State Street"), eyebrow "Guides", the index names the next guide and month from an optional `content.upcoming` or shows nothing (no schema change; the cockpit has no guide editor, recorded in §10). Retracted or not defects, recorded not built: EXP-018, EXP-023, EXP-037, SAV-004, META-004, A11Y-003, TP-B-02, TP-B-04 and the hearts part of SHR-002 (reasons in `docs/remediation-r1/FINAL.md`). Canon: `CLAUDE.md` v10 to v11 plus these eight entries.

---

## 2026-09-22 · R1 Wave 7: Saved, share, offline, forms, metadata

Source: `docs/remediation-r1/07_R1_W7_Saved_Share_Offline_Forms.md`; record `PROGRESS.md` ("Wave 7 deviations and findings"); evidence `docs/remediation-r1/evidence/w7-*.png`. Commit `d060f1e feat(r1-w7): saved counts and labels, recipient context, offline shell, form validation, branded emails, per-page metadata`.

Saved: the tab badge now counts Want to go only, with "N to go, M been" under the toggle. "Did you make it?" is labelled from the event's own Santa Barbara time and asks once. The empty state links "Browse today" and "Read a guide", and undated saves group under "Places and regulars". Recipient pages: /s, /p and /r share one frame (wordmark, explainer line, "Open SB Daymaker"), and past items are marked everywhere by one `isOver` rule. Offline: it works for the first time. The service worker had never registered, because it waited for a `load` event that had already fired. /saved now serves its cached copy and renders from the device with titles remembered at save time, and every other page falls back to /offline. A dropped connection reads as offline (`ThingsUnreachableError`), never as "No longer listed". Forms: submit validates inline and sets timing. Subscribe validates in the site's styling, keeps the address visible, and no longer reveals who is subscribed: a confirmed address gets a short "already on the list" email and the identical response. Emails: confirm and restore use the edition template with a plain-text alternative, and the cadence lives in `lib/edition/cadence.ts`. Metadata: all nine routes have their own og:title, og:image and description. Every page has header, main, nav and one footer landmark (Explore now uses the shared `PageFooter`). robots.txt keeps search crawlers out of /s/, /p/ and /r/ and lets link-preview bots in. An adversarial review (96 agents) confirmed 39 findings, all fixed and re-verified. This commit also carried early Wave 8 work: `lib/strings.ts`, `CADENCE_LINE`, and the build-failing dash check in `next.config.ts`. Open: Gmail rendering is Jim's to eyeball.

---

## 2026-09-22 · R1 Wave 6: Explore shell and cards

Source: `docs/remediation-r1/06_R1_W6_Explore_Shell_and_Cards.md`; evidence `docs/remediation-r1/evidence/cp4-{today,weekend,month}-390.png`. Commit `222d219 feat(r1-w6): url state, six-pill when row, one card anatomy, series cards, phone layout, search, slugs, hydration and load fixes`. Decisions D6, D7. CP4 approved 2026-09-22.

URL state: filter and horizon state live in the URL and the server reads it (`lib/exploreParams.ts`, `Object.hasOwn`), so a shared link opens on the same view. WHEN row: the six locked pills, each with its date range in its accessible name. Cards: one card anatomy on every horizon. The full-bleed RockTile is retired (small text over a photograph), and series collapse before week grouping ("Every Sunday, next Sep 27", "N dates"). Layout: no horizontal overflow and no text under 12px at 390, 320 and 200% zoom. Search: dedupes by series then title, chips events by date and venues as "Venue", and counts a transposition as one edit ("musuem" finds both museums, with "Did you mean museum"). Slugs: every in-app link emits the slug, and UUID URLs 308 from the page itself. Three backfill bugs were fixed (unpaged selects, a published-only taken-set, one failure aborting the sweep) and 106 more rows were slugged. Not-found: the root `app/not-found.tsx` is branded, and missing things and guides answer 404. Guides: the "Now" badge reads the server's clock (verified under TZ=UTC). Analytics: `lens_select` fires for all three doors and carries a dimension. Accessibility on / is 100. LCP on / is 8.3 s against the 4 s target (baseline 16.0 s). Hidden feed sections no longer fetch 53 images, the pool is stripped for browse, and the hero is fetched first. The remaining gap is Lighthouse's simulation against localhost, pending a deployed preview.

---

## 2026-09-22 · R1 Wave 5: pipeline and content quality

Source: `docs/remediation-r1/05_R1_W5_Pipeline_and_Content.md`; record `PROGRESS.md` ("Wave 5 deviations and carry-overs"). Commit `5b4764e feat(r1-w5): title cleaning, series keys, enrich rules, price notes, alt text, digest from live data, guide copy` (the spec's subject said "image waterfall"; that task carried over). Decisions D4, D6, D9, D12, D13.

Titles: `ingest/clean.ts` splits "Title | Venue", expands acronyms from `ingest/data/acronyms.json` (LOTG becomes "Library on the Go"), calms shouting titles and lifts addresses out of blurbs. 563 rows changed; zero published titles now contain a pipe or "LOTG"; 532 rows gained a `venue_name`. Series: `ingest/series.ts` writes `series_key` for every dated row (1,928 rows, 940 series, 198 with more than one date). The key is title plus venue, without the weekday the prose asked for, because the weekday split the Arts and Crafts Show and gave Recreation Swim 18 keys. A cadence sentence is derived from the dates. Blurbs: `ingest/blurbRules.ts` states the blurb rules as tests, and the enrich prompt carries them. A blurb that repeats the title, names the wrong weekday, holds an address or uses a banned phrase is rejected at landing; length alone is not a defect. Display: cards read `blurb` and detail reads `blurb_long ?? blurb`. One `priceLabel()` puts `price_note` first, with a "$ under 15" key. Outbound buttons name the destination, and photos carry title-plus-venue alt. Digest: /digest/sample renders the coming send window from live data (D12). Guides: one Funk Zone block count, a walk line per guide, stale "Right now" hidden after 45 days, and the deliberate-error line retired (D13). Dashes: D9 is enforced in source. 27 files were corrected, and `scripts/check-emdash.mjs` fails on U+2013 too. Carried over: the AI re-enrichment batch run and the W5.7 image waterfall change.

---

## 2026-09-22 · R1 Wave 4: where things are

Source: `docs/remediation-r1/04_R1_W4_Where_Things_Are.md`; record `PROGRESS.md` ("Wave 4 deviations"). Commit `a9a7f97 feat(r1-w4): single area module, place door filters, area backfill, near me and plan consume it`. Decisions D5, D11. CP3 approved 2026-09-22.

Area module: `lib/areas.ts` is the single area vocabulary, with 8 areas (one label and one short form each), "Anywhere in SB", and `areaForThing()` resolving neighborhood then `nearby_zone`, never "other". `lib/doorZones.ts` is a thin adapter; `lib/zones.ts` keeps only the stored 6-value `nearby_zone` mapping. Door: the Place door now filters. Tile counts come from the corrected pool, zero-count tiles show disabled, and "Show the closest matches" says what it relaxed. Plan: its engine uses the 8 areas end to end. Saved: Near Me lists the same 8 plus "Anywhere in SB", makes the chosen area its own top group ("Funk Zone, 2 of 10"), and falls back from geolocation after 6 seconds. Backfill (CP3): 634 of 919 area-less rows resolved from seeded venues, single-venue sources, the venue dictionary and unambiguous postcodes. There was no paid geocoding, because the placeholder address would have invented an area. Published rows with no area fell from about 500 to 26, 8 of them public and live. Detail pages: they carry the module label. The Natural History Museum and Puesta del Sol resolve to Mission and Riviera, and Oak Park to Upper State. No Near Me on Explore (D11).

---

## 2026-09-22 · R1 Wave 3: Plan integrity

Source: `docs/remediation-r1/03_R1_W3_Plan_Integrity.md`. Commit `d3e1f88 fix(r1-w3): plan honors meals and area, honest notes, badge component, horizon to 31 days`. Decision D3.

Meals: `isFood()` excludes civic rows, food-service titles (food bank, pantry, meal service, soup kitchen), library programming, and free food events outside an allowlist (market, tasting, pop-up, food truck, happy hour). 47 rows were recategorized out of food. Notes: one notes reducer (`lib/plan/notes.ts`) keys notes by subject, so a gap is stated once. A draft under two stops says so, and an unfillable block shows "Nothing found for [block] yet". Regenerate excludes the current draft and says when the pool is exhausted. Area: the chosen area outranks area-less candidates, at most one area-less stop rides along, and walking days check coordinates against the area box. The draft says "We widened beyond [area] to fill the day". Horizon: 31 days. Badges: a shared `Badge` component fixed "Cookingdinner", "AFTERNOONNOW" and "THE CORENOW". Copy: "Seven quick questions"; "Night" is "Evening"; a stop is "From your saves" or "Suggested", never both. Analytics: `plan_built` carries `stops` and `notes` counts.

---

## 2026-09-22 · R1 Wave 2: retire the dead, gate the pick

Source: `docs/remediation-r1/02_R1_W2_Retire_and_Gate.md`; record `PROGRESS.md` ("Wave 2 deviations"); evidence `docs/remediation-r1/evidence/w2-*.png`. Commits `9eebcac feat(r1-w2): archive finished events, civic flag, real pick eligibility, eyebrow and nearby fixes` and `62bd8b2 docs(r1): record the iCloud git-object blocker holding up the push`. Decisions D2, D3, D14. CP1 approved 2026-09-21; CP2 approved 2026-09-22.

Retirement: `ingest/retire.ts` runs nightly from `ingest/run.ts` and archives an event 7 days after it ends (`retire:dryrun` / `retire:apply`). 1,017 rows were archived at CP1, then 4 more, taking published from 1,529 to 508. Archived pages: detail pages stay reachable with "already happened on [date]", noindex and a working heart. Out-of-month dates print the year, and the Verified stamp needs a confirmation within 90 days. The shared-list page resolves ids directly. Civic: `ingest/civic.ts` flags civic meetings and closure notices at land time (rules as data; 136 rows backfilled), and the pool, search and Plan exclude them. The city calendar is title-tested rather than blanket-flagged, which kept 23 of its 40 real activities. Pick: CP2 applied `hero_eligible` (49 of 512 rows lost it). The pick now requires eligibility, a real address and a start no more than 30 minutes past. The bare fallback is gone, and the pick is computed before the filters. Eyebrow: "Gray day move" needs genuinely gray weather and an indoor pick. Nearby list: dedupes by title, drops civic and finished rows, and caps at 5. Cockpit: the Live catalog gained Archived and Civic filters. Blocked: pushing the branch (iCloud evicted git objects).

---

## 2026-09-21 · R1 Wave 1: stop the loss

Source: `docs/remediation-r1/01_R1_W1_Stop_the_Loss.md`; baseline `docs/remediation-r1/BASELINE.md`; record `PROGRESS.md`; evidence `docs/remediation-r1/evidence/w1-*.png`; audits `docs/audits/SB_Daymaker_UI_Audit_Log.md` and `docs/audits/2026-09-21-technical-pass.md`. Commit `8a3499b fix(r1-w1): saves resolve by id, pool excludes finished events, share link visible, revalidate after ingest` (it also adds the audits and every R1 spec). Decisions D1, D2 (query half).

The problem: the site was deleting saves. `getPublishedThings()` had no status filter, no date filter and no paging. Once the R1 DDL let anon read archived rows, it served 436 archived rows and lost every evergreen place past PostgREST's 1,000-row ceiling, and /saved deleted anything it could not see. Saves: they now resolve by id through `getThingsByIds()` (published and archived, in chunks of 200) and are never auto-deleted (D1). Unknown ids render "No longer listed" with Remove, and archived ones "Already happened, [date]". `SavesProvider` no longer wipes a list it cannot parse (`sbd.saves.v1.unreadable`). Pool: it filters `status = 'published'` explicitly, drops events finished more than a day ago (`lib/poolFilter.ts`), and pages with `.range()` until the exact count is met, because a larger limit cannot lift the ceiling. `getStopThingMap()` shares the status rule. Backup: the panel shows from the first save. Sharing: a `useShareLink()` sheet shows the URL with Copy whenever native share does not take it, on all five share surfaces. Freshness: `POST /api/revalidate` runs after each successful ingest, and ISR dropped from 600 to 300 s. Baseline: 864 tests in 59 files (the index's 597 was stale); LCP 16.0 s.
