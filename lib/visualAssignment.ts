 ...
 *  one of these landmarks (reusing the Phase 1 marquee registry's own matcher —
 *  `ingest/marqueeVenues.ts`'s `matchMarqueeVenue`, so this never drifts from the
...
/** Roughly 1-in-8 things get the big-type treatment even when a motif pool exists —
 *  matching the mockup's own ~1-in-10 ratio, and its own demonstrated behavior: the
