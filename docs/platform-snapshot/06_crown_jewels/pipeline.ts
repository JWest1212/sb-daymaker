/* ============================================================================
 * CROWN JEWEL — verbatim copy of lib/pipeline.ts
 * (SB Daymaker platform snapshot @ caa7302, 2026-07-03. Read-only copy.)
 *
 * ROLE: Legacy admin-gated submission->publish pipeline exposed via the 
 * runPipeline server action. AUDIT FOCUS (Constraint 3): this is the ONE path 
 * where @anthropic-ai/sdk (via lib/enrich.ts) is import-reachable from a 
 * request handler; real ingestion moved to ingest/run.ts, making this a 
 * dormant duplicate that keeps AI technically reachable from an admin action.
 * ==========================================================================*/
