...
1. **Setup screen** — a flagship **"Make My Day"** express button (instant ready-made day), then **five simple questions** (When · Which parts of the day · Who · Vibe · Where), a **"Pin saved spots"** hook, and a **"Show me my day"** CTA. A top-right **My plans** drawer lists saved itineraries.
...
|---|---|---|
| **When** | Today / Tomorrow / Pick a date | a date (drives dated-event availability) |
| **Parts of day** (multi) | Morning / Afternoon / Evening / Night | `tod`: `morning` / `afternoon` / `evening` / **`late`** (+ midday bridge per §3.3) |
| **Who** (single) | Solo / Couple / Family / Friends | soft signal for ranking (e.g. Family ⇒ favor `family_day`-tagged, stroller-OK); not a hard filter |
| **Vibe** (multi, 4) | Outdoors / Wine & Food / Arts & Culture / Date Night | `occasion_tag`: `outdoors_active` / `wine_food` / `arts_culture` / `date_night` |
| **Where** (single) | Downtown / Funk Zone / Waterfront / The Mesa / Montecito / Goleta / **Anywhere** | `nearby_zone`: `downtown` / `funk` / `waterfront` / `mesa` / `montecito` / `goleta` / **none** |

The four vibe tags are a deliberate **subset** of the ten `occasion_tag`s (don't show all ten — keep setup simple).
...
### Phase 3 — Setup screen (Make My Day + five questions + saved hook)
**Goal:** the full setup screen, matching mockup **State 1**.

- **Make My Day** flagship card (terracotta→gold, sun glyph, *"Short on time?"* eyebrow). Tapping it runs `buildDay(default…)` and routes to the spine (Phase 4).
- **The five question groups** (Appendix B for layout): When (single), Parts of day (multi, full-width 4-up, **no Midday button**), Who (single, full-width 4-up), Vibe (multi, 4 tags), Where (single, **two equal rows** 4+3, includes Goleta + Anywhere). Selection state per Appendix C. Multi vs single per §4.
...
**Acceptance:** matches State 1; selections toggle correctly (single clears siblings across both rows for Where; multi toggles); saved hook hidden when nothing is saved; both Make My Day and the CTA land on the spine; keyboard + focus + 44px throughout.
**Commit:** `feat(plan): setup screen — Make My Day, five questions, saved hook`
...
## Appendix B — Setup layout cribsheet
...
- **Full-width rows** (Parts of day; Who): equal-width columns (`flex:1`), icon-over-label, ~12px label.
- **Where**: two rows, 4 then 3, equal-width columns; single-select must clear across **both** rows.
...
interface PlanAnswers {
  ...
  vibes: ('outdoors_active'|'wine_food'|'arts_culture'|'date_night')[];
