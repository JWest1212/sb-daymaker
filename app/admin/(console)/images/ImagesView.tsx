...
import { TIER_WORD, DISMISS_SKIP_FOR_NOW, DISMISS_KEEP_MOTIF_FOREVER } from "../../ui/vocab";
...
// S7/D5, resumable bulk runs (mockup r3). Persisted after every chunk so an
// interrupted auto-free/auto-Google pass (tab closed, browser crash) can pick
// back up instead of silently vanishing; cleared the moment a run concludes
// on its own (finishes, hits the cap, or errors out), since only an actual
// interruption should leave a manifest behind for next mount to find.
const IMAGE_RUN_KEY = "sbd-cockpit:imagerun";
const IMAGE_RUN_MAX_AGE_MS = 7 * 86_400_000;

interface ImageRunFilterState { filter: Filter; search: string; noAddressOnly: boolean; }
interface ImageRunManifest {
  kind: "free" | "google";
  filterState: ImageRunFilterState;
  ids: string[]; // remaining, unprocessed ids
  done: number;
  total: number;
  startedAt: number;
}
interface ImageRunResume { ids: string[]; done: number; total: number; startedAt: number; filterState: ImageRunFilterState; }

function readImageRunManifest(): ImageRunManifest | null {
  try {
    const raw = localStorage.getItem(IMAGE_RUN_KEY);
    if (!raw) return null;
    const m = JSON.parse(raw) as ImageRunManifest;
    if (!m?.ids?.length || Date.now() - m.startedAt > IMAGE_RUN_MAX_AGE_MS) {
      localStorage.removeItem(IMAGE_RUN_KEY);
      return null;
    }
    return m;
  } catch {
...
function writeImageRunManifest(m: ImageRunManifest) {
  try { localStorage.setItem(IMAGE_RUN_KEY, JSON.stringify(m)); } catch { /* storage unavailable, resumability just degrades */ }
}
function clearImageRunManifest() {
  try { localStorage.removeItem(IMAGE_RUN_KEY); } catch { /* ignore */ }
}
...
export function ImagesView({ initial }: { initial: ImagesDeskData }) {
  ...
  // S7/D5, an interrupted bulk run found on mount (<7 days old); null once
  // resumed or dismissed. Read once on mount, this tab only, no polling.
  const [runManifest, setRunManifest] = useState<ImageRunManifest | null>(null);
  useEffect(() => { setRunManifest(readImageRunManifest()); }, []);
  ...
  // S7/D5, an optional `resume` continues a prior interrupted pass over its
  // own remaining ids instead of re-scanning the current view from scratch;
  // `total`/`startedAt` carry over so progress and the 7-day age window stay
  // anchored to when the pass actually began.
  const autoAssignAll = useCallback(async (resume?: ImageRunResume) => {
    const targets = resume ? resume.ids : visible.map((r) => r.id);
    if (!targets.length || autoRunning) return;
    if (!resume) {
      const okGo = window.confirm(
    ...
    const total = resume?.total ?? targets.length;
    const startDone = resume?.done ?? 0;
    const startedAt = resume?.startedAt ?? Date.now();
    const filterState: ImageRunFilterState = resume?.filterState ?? { filter, search, noAddressOnly };
    const snapshot = rows.filter((r) => targets.includes(r.id));
    setRunManifest(null);
    setAutoRunning(true);
    ...
    try {
      for (let i = 0; i < targets.length; i += 60) {
        ...
        for (let j = 0; j < locateIds.length; j += 25) {
          ...
          setBulkProgress(`Locating ${startDone + i + j + batch.length}/${total}…`);
        ...
        for (let j = 0; j < searchIds.length; j += 8) {
          ...
          setBulkProgress(`Searching ${startDone + i + j + batch.length}/${total}…`);
        ...
        const doneNow = startDone + Math.min(i + chunk.length, targets.length);
        setBulkProgress(`Assigning ${doneNow}/${total}…`);
        ...
        writeImageRunManifest({
          kind: "free", filterState, ids: targets.slice(i + chunk.length), done: doneNow, total, startedAt,
        });
      }
      clearImageRunManifest();
    } finally {
  ...
  }, [visible, rows, autoRunning, processAutoResults, showToast, filter, search, noAddressOnly]);
  ...
  const autoGoogleAll = useCallback(async (resume?: ImageRunResume) => {
    const targets = resume ? resume.ids : visible.map((r) => r.id);
    if (!targets.length || autoRunning) return;
    if (!resume) {
      const okGo = window.confirm(
    ...
    try {
      for (let i = 0; i < targets.length; i += 60) {
        const chunk = targets.slice(i, i + 60);
        processed = startDone + Math.min(i + chunk.length, targets.length);
        setBulkProgress(`Google ${processed}/${total}…`);
        ...
        writeImageRunManifest({
          kind: "google", filterState, ids: targets.slice(i + chunk.length), done: processed, total, startedAt,
        });
      }
      clearImageRunManifest();
    } finally {
  ...
  const resumeImageRun = useCallback(() => {
    if (!runManifest) return;
    const m = runManifest;
    setFilter(m.filterState.filter);
    setSearch(m.filterState.search);
    setNoAddressOnly(m.filterState.noAddressOnly);
    setRunManifest(null);
    const resume: ImageRunResume = { ids: m.ids, done: m.done, total: m.total, startedAt: m.startedAt, filterState: m.filterState };
    if (m.kind === "free") autoAssignAll(resume); else autoGoogleAll(resume);
  }, [runManifest, autoAssignAll, autoGoogleAll]);

  const dismissImageRun = useCallback(() => {
    clearImageRunManifest();
    setRunManifest(null);
  }, []);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // QW4, U triggers the visible toast's undo (if any) while it shows.
      if (k === "u" && toast?.undo) { e.preventDefault(); toast.undo(); return; }
  ...
  }, [pageItems, activeIdx, dismissedSuggestions, cyclePick, applySelected, applyBest, approveVenue, doGoogleFetch, autoGoogleOne, doFreeSearch, ackAsIs, toast]);
    ...
    <div className="wrap">
      <main>
        ...
        {runManifest ? (
          <div className="resume" role="status">
            <p>
              <b>{runManifest.kind === "free" ? "Auto-free" : "Auto-Google"} run interrupted.</b>{" "}
              Stopped at <span className="mono">{runManifest.done} of {runManifest.total}</span>, finished work is saved.
            </p>
            <span className="spacer" />
            <button type="button" className="rb" onClick={resumeImageRun} disabled={autoRunning}>Resume run</button>
            <button type="button" className="btn btn-quiet btn-sm" onClick={dismissImageRun}>Dismiss</button>
          </div>
        ) : null}
            ...
            return (
              <article key={row.id} className={`card ${i === activeIdx ? "is-active" : ""}`} onClick={() => setActive(i)}>
                <div className="card-grid">
                  ...
                  <div className="body">
                    <div className="row1">
                      <span className={`tier t${row.happening_tier}`}>{TIER_WORD[row.happening_tier] ?? "T?"}</span>
                    ...
                    <div className="actions pt">
                      ...
                      <button className="btn btn-quiet btn-sm" disabled={busyId === row.id} onClick={() => ackAsIs(row)}>
                        {DISMISS_KEEP_MOTIF_FOREVER} <span className="k">M</span>
                      </button>
                    </div>
                  </div>
                </div>
      ...
      <aside className="side">
        <div className="panel">
          ...
          <div className="keys">
            ...
            <div className="kr"><span className="kk">M</span> Keep motif forever (dismiss)</div>
          </div>
        </div>
        <div className="panel">
      ...
      {toast ? (
        <div className="toast show" role="status">
          ...
          {toast.undo ? (
            <>
              <span className="bar" aria-hidden="true"><i style={{ "--toast-ms": "3600ms" } as React.CSSProperties} /></span>
              <button type="button" className="undo" onClick={toast.undo}>Undo</button>
            </>
          ) : null}
        </div>
      ) : null}
