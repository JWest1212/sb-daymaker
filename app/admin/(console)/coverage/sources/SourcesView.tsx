...
import { sourceHealth } from "@/lib/review";
  ...
  return (
    <div className="wrap" style={{ display: "block", maxWidth: 1080 }}>
      <div className="sweep-head">
        <Link href="/admin/coverage" className="back-link">&larr; Coverage</Link>
              ...
              {sources.map((r) => (
                <tr key={r.key} className={r.status === "active" ? undefined : "rr-inactive-row"}>
                  <td>
                    ...
                    <div className="src">{r.key}</div>
                  </td>
                  <td>{titleCaseLabel(r.lane)}{r.parse_method ? ` · ${r.parse_method}` : ""}</td>
                  <td>{r.authority.toFixed(2)}</td>
                  <td>
                    <span className={`yield${r.status === "active" && sourceHealth(r) === "below_baseline" ? " below" : ""}`}>
                      {r.last_yield ?? "·"} / {r.expected_yield || "·"}
                    </span>
                  </td>
      ...
      <Sheet
        ...
        onClose={closeSheet}
        titleId="srcTitle"
        title={sheetTarget === "new" ? "Add a candidate source" : sheetTarget ? `Edit ${(sheetTarget as SourceRow).label}` : ""}
        footer={
          <>
