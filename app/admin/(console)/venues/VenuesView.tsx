...
type VenueTab = "matches" | "nomatch" | "placeid" | "registry" | "dogfriendly";
...
function VenueDetailSheet({
  ...
  return (
    <Sheet open onClose={onClose} titleId="vTitle" title={venue.display_name} wide>
                ...
                <div className="pickrow" key={t.id}>
                  <div>
                    <div className="ttl">
                      ...
                      <span className={`tier t${t.happening_tier}`}>{TIER_WORD[t.happening_tier] ?? "T?"}</span>
                    </div>
...
export function VenuesView({ initial }: { initial: VenuesData }) {
  ...
  const [activeTab, setActiveTab] = useState<VenueTab>("matches");

  // QW1, honest count + client pagination for "Matches to review" (previously
  // a silent .slice(0, 40) with no indication more existed beyond it).
  ...
  const doAck = useCallback(async (thingId: string) => {
    ...
    if (res?.ok) { removeCatcherItem(thingId); showToast("Kept on motif forever"); }
  ...
  const matchesTotalPages = Math.max(1, Math.ceil(visibleMatches.length / MATCHES_PAGE_SIZE));
  const matchesPageClamped = Math.min(matchesPage, matchesTotalPages);
  const matchesPageItems = visibleMatches.slice(
    (matchesPageClamped - 1) * MATCHES_PAGE_SIZE,
    matchesPageClamped * MATCHES_PAGE_SIZE,
  );
  ...
  const venuesNeedingPhotosCount = data.venues.filter((v) => v.approvedPhotos.length === 0).length;

  const TABS: { key: VenueTab; label: string; count: number }[] = [
    { key: "matches", label: "Matches to review", count: visibleMatches.length },
    { key: "nomatch", label: "No confident match", count: data.noMatchCatcher.length },
    { key: "placeid", label: "Place ID lookup", count: visibleStrongMatches.length + visibleWeakMatches.length },
    { key: "registry", label: "Venue registry", count: venuesNeedingPhotosCount },
    { key: "dogfriendly", label: "Dog Friendly", count: dogSuggestedUnconfirmedCount },
  ];
  ...
  return (
    <div className="wrap" style={{ display: "block", maxWidth: 1180 }}>
      ...
      <div className="venue-tabs" role="tablist" aria-label="Venues sections">
        {TABS.map((t) => (
          <button
            key={t.key}
            type="button"
          ...
          >
            {t.label}
            {t.count > 0 ? <span className="vtab-n">{t.count}</span> : null}
          </button>
        ))}
      </div>
      ...
      <div className="vsection">
        <h2 className="vsection-title">
          Matches to review
          {visibleMatches.length > 0
            ? ` · showing ${(matchesPageClamped - 1) * MATCHES_PAGE_SIZE + 1}-${Math.min(matchesPageClamped * MATCHES_PAGE_SIZE, visibleMatches.length)} of ${visibleMatches.length}`
            : " (0)"}
        </h2>
        {visibleMatches.length === 0 ? (
          ...
          <div className="matchlist">
            {matchesPageItems.map((m) => (
              <div className="pickrow" key={m.thing_id}>
                <div>
                  <div className="ttl">{m.title}</div>
                  <div className="pm">{TIER_WORD[m.happening_tier] ?? "T?"} · {m.address} · proposed: <span className="venuename">{m.venue_display_name}</span> (score {m.score.toFixed(1)})</div>
                </div>
                <div className="btnrow">
                  <button className="btn btn-approve btn-sm" onClick={() => approveMatch(m)}>Approve</button>
                  <button className="btn btn-quiet btn-sm" onClick={() => dismissMatch(m.thing_id)}>{DISMISS_SKIP_FOR_NOW}</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {matchesTotalPages > 1 ? (
          <div className="pager">
            <button disabled={matchesPageClamped <= 1} onClick={() => setMatchesPage(matchesPageClamped - 1)}>← Prev</button>
            <span>Page {matchesPageClamped} of {matchesTotalPages}</span>
            <button disabled={matchesPageClamped >= matchesTotalPages} onClick={() => setMatchesPage(matchesPageClamped + 1)}>Next →</button>
          </div>
        ) : null}
      </div>
      ) : null}
        ...
        {filteredCatcher.length === 0 ? (
          ...
          <div className="matchlist">
            {catcherPageItems.map((t) => (
              <div className="pickrow" key={t.id}>
                <div style={{ width: "100%" }}>
                  <div className="ttl">{t.title} <span className={`tier t${t.happening_tier}`}>{TIER_WORD[t.happening_tier] ?? "T?"}</span></div>
