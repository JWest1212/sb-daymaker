...
interface Pending { timer: ReturnType<typeof setTimeout>; commit: () => Promise<void>; }
...
export function ReviewQueue({ initial }: { initial: CockpitData }) {
  ...
  /** Splice removed items back into the queue at their original indices,
   *  the same restore mechanism Undo uses (also reused on a failed commit). */
  const restoreRemoved = useCallback((removed: { item: QueueRow; index: number }[]) => {
    setQueue((q) => {
      const next = [...q];
   ...
   *  failVerb labels the failure toast ("Publish"/"Reject") if the eventual
   *  commit's response comes back not-ok; the item is restored in that case. */
  const commitAction = useCallback(
    (ids: string[], removed: { item: QueueRow; index: number }[], verb: string, run: () => Promise<Response>, failVerb: "Publish" | "Reject") => {
      const key = ids.join(",");
      const label = removed.length === 1 ? removed[0].item.title : `${removed.length} items`;
      ...
      const commit = async () => {
        ...
        if (!res?.ok) {
          ...
          showToast(`${failVerb} failed for ${label}, kept in queue`);
        }
      };
      const timer = setTimeout(commit, COMMIT_MS);
  ...
  const approve = useCallback((item: QueueRow) => {
    ...
    if (item.overlay_id) {
      commitAction([item.id], [{ item, index }], "Replaced live", () =>
        post("/api/review/approve", { overlay_id: item.overlay_id, edits: edits_, photo, hero_eligible: isHero(item) }), "Publish");
      return;
    }
  ...
  const reject = useCallback((item: QueueRow) => {
    ...
    if (item.overlay_id) {
      commitAction([item.id], [{ item, index }], "Discarded edit", () =>
        post("/api/review/reject", { overlay_id: item.overlay_id, id: item.edit_of, reason: "founder discard" }), "Reject");
      return;
    }
    commitAction([item.id], [{ item, index }], "Rejected", () =>
      post("/api/review/reject", { id: item.id, reason: "founder reject" }), "Reject");
  }, [queue, commitAction]);
  ...
  const bulkGreen = useCallback(() => {
    ...
    commitAction(ids, removed, "Published", () => post("/api/review/approve", { ids }), "Publish");
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // checked before the edit-mode branch so it works in either mode, but
      // still behind the focus guard above like every other shortcut here.
      if (e.key.toLowerCase() === "u" && toast?.undo) { e.preventDefault(); toast.undo(); return; }
  ...
  }, [visible, activeIdx, editingId, queue, approve, reject, bulkGreen, cycle, startEdit, exitEdit, toggleHero, toast]);
    ...
    <>
      ...
      <div className="wrap">
        ...
        <aside className="side">
          ...
          <RejectedPanel onToast={showToast} />
          ...
          <div className="panel">
            ...
            <div className="keys">
              ...
              <div className="kr"><span className="kk">R</span> Reject, restorable for 14 days</div>
              ...
              <div className="kr"><span className="kk">U</span> Undo, while the toast shows</div>
            </div>
          </div>
        </aside>
      </div>
      ...
      {toast ? (
        <div className="toast show" role="status">
          ...
          {toast.undo ? (
            <>
              <span className="bar" aria-hidden="true"><i style={{ "--toast-ms": `${COMMIT_MS}ms` } as React.CSSProperties} /></span>
