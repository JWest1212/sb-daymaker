...
import { TIER_WORD, TIER_WORD_PLURAL } from "../../ui/vocab";
...
export function CatalogView({ initial }: { initial: CatalogResult }) {
  ...
  // S3b, the draft zone's dirty count (the live zone's photo picker applies
  // instantly and isn't part of this, it has its own per-action Undo toast).
  const editDirtyCount = editing && draft ? [
    draft.title !== editing.title,
    draft.blurb !== (editing.blurb ?? ""),
    draft.blurb_long !== (editing.blurb_long ?? ""),
    draft.neighborhood !== (editing.neighborhood ?? ""),
    JSON.stringify([...draft.tags].sort()) !== JSON.stringify([...editing.tags].sort()),
  ].filter(Boolean).length : 0;

  // Cancel (and Esc/scrim, same handler via Sheet's onClose) only confirms
  // when there's something to lose, and says so accurately: the live zone's
  // photo changes are already saved, only the draft zone is at stake.
  const cancelEdit = useCallback(() => {
    if (editDirtyCount > 0 && !window.confirm(
      `Discard ${editDirtyCount} unsaved change${editDirtyCount === 1 ? "" : "s"} to the draft fields?\n\nAny photo change you made is already live and unaffected.`,
    )) return;
    setEditing(null);
    setDraft(null);
  }, [editDirtyCount]);
  ...
  // S3c, Delete = unpublish/archive (API-02 is already a soft archive, not a
  // hard delete), so it gets the same Undo-via-unarchive affordance as bulk
  // archive: reversible action, reversible UI.
  const del = useCallback(async (r: CatalogRow) => {
    ...
    if (res?.ok) {
      ...
      const undo = async () => {
        const r2 = await fetch("/api/admin/catalog/bulk", {
          method: "POST", headers: { "content-type": "application/json" },
          body: JSON.stringify({ ids: [r.id], op: "unarchive" }),
      ...
      showToast(`Removed "${r.title}" from the site`, undo);
    } else {
    ...
    <div className="wrap" style={{ display: "block", maxWidth: 1180 }}>
        ...
        <div className="filterbar" role="group" aria-label="Filter by tier">
          {(["all", "1", "2", "3"] as Tier[]).map((t) => (
            <button key={t} className="filt" aria-pressed={tier === t} onClick={() => setTier(t)}>
              {t === "all" ? "All" : TIER_WORD_PLURAL[Number(t)]}
            </button>
          ))}
        </div>
      ...
      {selected.size > 0 ? (
        <div className="bulkbar" role="group" aria-label="Bulk actions">
          ...
          <span className="bulknote">Selection clears when the page changes</span>
      ...
      <Sheet
        ...
        onClose={cancelEdit}
        titleId="ceTitle"
        title="Edit live thing"
        footer={
          <>
            <button
              className="btn btn-quiet"
              style={{ marginRight: "auto" }}
              onClick={() => editing && doRedraft(editing.id)}
              title="Queues a fresh AI blurb/tags draft for tonight's worker, no AI call now"
            >
              Redraft blurb + tags tonight
            </button>
            {editDirtyCount > 0 ? <span className="dirty">&#9679; {editDirtyCount} unsaved change{editDirtyCount === 1 ? "" : "s"}</span> : null}
            <button className="btn btn-edit" onClick={cancelEdit}>Cancel</button>
        ...
        {editing && draft ? (
          <>
            <div className="zone live">
              <div className="zone-head">⚡ Photo &middot; applies to the live site immediately</div>
              <div className="zone-body">
                <CatalogImagePicker
                  ...
                  onToast={showToast}
                />
                <p className="zone-note">Each apply goes live at once and gets its own Undo toast.</p>
              </div>
            </div>

            <div className="zone draft">
              <div className="zone-head">&#9998; Details &middot; saved together when you press Save</div>
              <div className="zone-body">
                ...
                <div className="gatebox">These fields save together when you press Save. (Start time isn&apos;t editable here; to change one, reject &amp; re-ingest in the Queue.)</div>
              </div>
            </div>
          </>
        ) : null}
