...
export async function sendDigest(sb: SupabaseClient, s: DigestSummary): Promise<void> {
  ...
  const [{ count: queued }, dropsRes, runsRes, failedEditionsRes] = await Promise.all([
    ...
    // last 14 days so a months-old failure doesn't nag forever. Every OTHER status
    // (draft/approved/skipped-on-hold) sends at its normal time regardless, so there's
    // nothing to proactively alert on for those.
  ...
  <div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto;color:#241C16">
    ...
    ${failedEditions.length ? `<a href="${site}/admin/edition-draft" style="display:inline-block;margin-left:10px;background:#B0592A;color:#FCFAF5;text-decoration:none;font-weight:600;padding:11px 20px;border-radius:999px">Review the edition →</a>` : ''}
  </div>`;
